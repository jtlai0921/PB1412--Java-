...
public class ElectronicClock extends JFrame {
    ...
    private void getTime() {
        ...
        int hour = calendar.get(Calendar.HOUR_OF_DAY); // 獲得目前的小時
        int minute = calendar.get(Calendar.MINUTE); // 獲得目前的分鐘
        int second = calendar.get(Calendar.SECOND); // 獲得目前的秒鐘
        hour1Label.setIcon(numbers[hour / 10]);// 利用商獲得小時第一位圖片
        hour2Label.setIcon(numbers[hour % 10]);// 利用餘數獲得小時第二位圖片
        minute1Label.setIcon(numbers[minute / 10]);// 利用商獲得分鐘第一位圖片
        minute2Label.setIcon(numbers[minute % 10]);// 利用餘數獲得小時第二位圖片
        second1Label.setIcon(numbers[second / 10]);// 利用商獲得秒鐘第一位圖片
        second2Label.setIcon(numbers[second % 10]);// 利用餘數獲得小時第二位圖片
    }
