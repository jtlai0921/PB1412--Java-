...
public class SuperFun extends JFrame {
    ...
    protected void do_button_actionPerformed(ActionEvent e) {
        ...
        for (int i = 0; i < times; i++) {
            for (int j = 0; j < 5; j++) {// 在1~35中隨機選擇5個數字
                List<Integer> list = new ArrayList<Integer>();
                for (int k = 1; k < 36; k++) {
                    list.add(k);// 將1~35增加到列表中
                }
                int number = list.get(new Random().nextInt(list.size()));// 隨機選擇一個數字
                String luckNumber = number < 10 ? "0" + number : "" + number;// 格式化數字
                sb.append(luckNumber + " ");// 向sb中增加數字
                list.remove(new Integer(number));// 刪除選擇的數字，這樣就避免了重複
            }
            sb.append("\t\t");
            for (int j = 0; j < 2; j++) {// 在1~12中隨機選擇2個數字
                List<Integer> list = new ArrayList<Integer>();
                for (int k = 1; k < 13; k++) {
                    list.add(k);// 將1~12增加到列表中
                }
