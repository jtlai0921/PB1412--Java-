...
public class BubbleSort extends JFrame {
    ...
    public BubbleSort() {
        ...
        JButton button_1 = new JButton("排序法");
        button_1.setActionCommand("排序");
        ...
        for (int i = 1; i < array.length; i++) {
            ...
            for (int j = 0; j < array.length - i; j++) {
                if (array[j] > array[j + 1]) {
                    int temp = array[j];// 把第一個元素值保持到臨時變數中
                    array[j] = array[j + 1];// 把第二個元素值儲存到第一個元素單元中
                    array[j + 1] = temp;// 把臨時變數也就是第一個元素原值保持到第二個元素中
                }
                textArea2.append(array[j] + "  ");// 把排序後的陣列元素顯示到文字域中
            }
            textArea2.append("【");
