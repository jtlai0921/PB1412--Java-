...
public class FullScreenFrame extends JFrame {
    ...
    public FullScreenFrame() {
        ...
        JButton fullscreenButton = new JButton("全屏顯示");
        fullscreenButton.setFont(new Font("微軟雅黑", Font.PLAIN, 16));
    ...
    protected void do_fullscreenButton_actionPerformed(ActionEvent arg0) {
        ...
        Dimension dimension = toolkit.getScreenSize(); // 獲得使用者的顯示器大小
        double times = dimension.getHeight() / rect.height;// 獲得高度需要放大的倍數
        Image image = pdfPage.getImage((int) (rect.width * times), dimension.height, rect, null, true, true);// 設定圖片的大小
        if (pdfPanel != null) {
            pdfPanel.removeAll();
        }
        pdfPanel.add(new JLabel(new ImageIcon(image))); // 顯示產生的圖片
